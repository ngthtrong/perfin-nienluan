...
\subsection{Flutter và ngôn ngữ Dart cho phát triển ứng dụng di động}
\label{subsec:flutter_dart}

Flutter là một framework mã nguồn mở được phát triển bởi Google, cho phép xây dựng ứng dụng di động đa nền tảng (cross-platform) từ một mã nguồn duy nhất (single codebase), hỗ trợ cả Android và iOS \cite{ref15}. Flutter sử dụng ngôn ngữ lập trình Dart --- một ngôn ngữ hướng đối tượng, biên dịch AOT (Ahead-of-Time) cho hiệu năng cao và JIT (Just-in-Time) cho tính năng Hot Reload.

Lý do PerFin chọn Flutter:

\begin{itemize}
    \item \textbf{Single Codebase}: Một mã nguồn duy nhất chạy trên cả Android và iOS, giảm 40--50\% thời gian phát triển so với phát triển native riêng cho từng nền tảng.
    \item \textbf{Hot Reload}: Cho phép lập trình viên thấy ngay kết quả thay đổi mã nguồn trên thiết bị/giả lập mà không cần biên dịch lại toàn bộ ứng dụng, tăng tốc đáng kể chu kỳ phát triển.
    \item \textbf{Hệ thống Widget phong phú}: Flutter cung cấp thư viện widget đa dạng, tuân thủ Material Design (Android) và Cupertino (iOS), giúp xây dựng giao diện đẹp và nhất quán.
    \item \textbf{Hiệu năng gần native}: Flutter biên dịch trực tiếp sang mã máy (ARM) thay vì sử dụng bridge (như React Native), mang lại hiệu năng gần bằng ứng dụng native.
    \item \textbf{Hệ sinh thái packages}: Kho thư viện pub.dev cung cấp nhiều packages hỗ trợ cho các tính năng của PerFin: \texttt{fl\_chart} (biểu đồ), \texttt{image\_picker} (chọn ảnh), \texttt{record} (ghi âm), \texttt{provider/riverpod} (quản lý state).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{JavaScript full-stack}: Sử dụng JavaScript/TypeScript cho cả frontend (Flutter sử dụng Dart nhưng cấu trúc tương tự) và backend giúp giảm chi phí chuyển đổi ngữ cảnh cho nhóm phát triển.
...
\begin{itemize}
    ...
    \item \textbf{Visual Studio Code (VS Code)}: Trình soạn thảo mã nguồn mạnh mẽ với hệ sinh thái extension phong phú, hỗ trợ tốt cả Dart/Flutter (thông qua Flutter extension) và Node.js. Tích hợp terminal, debugger, và Git giúp tăng năng suất phát triển.
...
Đa nền tảng & Có & Có & Có & Có & Có (Flutter) \\ \hline
