...
Ngày kiểm kê: 16/07/2026  
Phạm vi: `demo/data/`, schema PostgreSQL qua migrations `001`–`008`, các model giao dịch/danh mục/ví.  
Trạng thái: **đã import và đối soát trên PostgreSQL demo ngày 16/07/2026**, sau khi thử hai lần trên database clone và tạo backup trước thay đổi.

## 1. Kết luận ngắn

`dataFinance.csv` đủ tốt để làm tập dữ liệu demo cho phần dữ liệu và giải thuật sau một bước chuẩn hóa có kiểm soát. Tệp có 5.265 dòng logic, không thiếu mô tả/số tiền/ngày/loại, không có ngày sai, không có sai khác giữa hai cột tiền và không có dấu thu/chi ngược. Pipeline mặc định giữ cả 5.265 giao dịch. Có 24 dòng giống hoàn toàn với dòng khác, nhưng nguồn không có ID hoặc giờ giao dịch để chứng minh đây là bản sao thay vì hai sự kiện thật cùng ngày.
...
- 24 dòng giống hoàn toàn và 621 giá trị vượt ngưỡng IQR chỉ được gắn cờ; muốn loại exact-match phải dùng tùy chọn rõ ràng;
...
|---|---:|---|---|---|
| `dataFinance.csv` | 328.050 byte | UTF-8 BOM, CSV, 5.271 dòng vật lý/5.265 bản ghi logic | `a9b7cf1b...94f` | Đã import sau validation, mapping và backup |
...
| Mô tả được chuẩn hóa Unicode/whitespace trong kế hoạch mặc định | 374 |

Các ô trống lớn ở hai cột category là có chủ ý: dòng thu để trống `Type Expenses`, dòng chi để trống `Type In come`. `Special` trống 4.978/5.265 dòng và chỉ nên xem là metadata tùy chọn.

### 4.2 Thời gian

- Min–max: `2022-01-01` đến `2026-07-15`.
...
- Tháng 07/2026 là tháng chưa hoàn tất nên không được so sánh trực tiếp với tháng đầy đủ.
...
|---:|---:|---:|---:|---:|---:|---:|
| 1.000 | 14.000 | 25.000 | 44.000 | 48.000.000 | 89.000 | 621 |
...
|---|---|---|---:|
| Chi | Ăn uống | `AC`, `AV`, `Coffee` | 2.251 |
| Chi | Di chuyển | `DC` | 491 |
| Chi | Giải trí | `Bida`, `Entertainment`, `G`, `NET`, `RL` | 660 |
| Chi | Thể thao | `Badmintion`, `Gym`, `Sport`, `Swimming` | 394 |
| Chi | Giáo dục | `English`, `Study` | 342 |
| Chi | Hóa đơn & Dịch vụ | `Utility` | 356 |
| Chi | Nhà cửa | `Home` | 150 |
| Chi | Sức khỏe | `Body`, `Mental` | 80 |
| Chi | Mua sắm | `CL` | 38 |
| Chi | Khác | trống, `Accident`, `Cho vay`, `Family`, `Ngu`, `Smoking` | 83 |
| Thu | Lương | `Salary` | 67 |
| Thu | Đầu tư | `Invest` | 3 |
| Thu | Khác | `Anh em`, `Cho vay`, `Clan`, `Family`, `Luck`, `Sell old things` | 350 |
...
Nguồn không có thông tin ví. Việc dồn 5.265 dòng vào `Tiền mặt` là quyết định demo, không phải kết luận rằng mọi giao dịch thực tế dùng tiền mặt.
...
Trước khi thay dữ liệu, một bản backup PostgreSQL custom-format đã được tạo và kiểm tra danh mục restore. Importer được chạy hai lần trên database clone: lần đầu thay 336 giao dịch bằng 5.265 dòng, lần hai thay 5.265 bằng đúng 5.265 dòng và giữ nguyên các tổng, qua đó xác nhận tính tái lập.
...
|---|---:|
| Giao dịch active | 5.265 |
| Thu / chi | 420 / 4.845 |
| Min–max | 01/01/2022 – 15/07/2026 |
| Tổng thu | 393.770.659 VND |
| Tổng chi | 393.176.659 VND |
| Net và số dư ví `Tiền mặt` | 594.000 VND |
| Dòng có provenance `dataFinance.csv` | 5.265 / 5.265 |
| Source row provenance phân biệt | 5.265 / 5.265 |
| Dòng bị reject | 0 |

Sau import, smoke test đầy đủ API/chat/database/media thật đạt 23/23; toàn bộ regression backend đạt 100/100 test.
