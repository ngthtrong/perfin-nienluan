...
test('transaction validation rejects non-finite/overflow amounts and malformed references', () => {
  assert.throws(() => validateTransactionPayload({ ...valid, amount: -50_000 }), /lớn hơn 0/);
  assert.throws(() => validateTransactionPayload({ ...valid, amount: '-50,000' }), /lớn hơn 0/);
  assert.throws(() => validateTransactionPayload({ ...valid, amount: 0 }), /lớn hơn 0/);
  assert.throws(() => validateTransactionPayload({ ...valid, amount: '0' }), /lớn hơn 0/);
