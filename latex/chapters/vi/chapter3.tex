...
Các mã \texttt{TC-FRxx-yy} dưới đây là định danh ca kiểm thử ở mức đặc tả. Một ca chỉ được xem là ``đã đo'' khi có dữ liệu đầu vào, kết quả mong đợi, môi trường và log tương ứng tại mục~\ref{sec:testing}.
...
\widereportfigure{14-usecase-overview.pdf}{Sơ đồ ca sử dụng tổng thể của PERFIN. Người dùng là tác nhân chính của toàn bộ mười hai chức năng; Dịch vụ AI ngoài và Worker nền chỉ tham gia các ca liên quan và nằm ngoài biên tin cậy dữ liệu.}{fig:usecase-overview}

Hình~\ref{fig:usecase-overview} tổng hợp mười hai chức năng FR-01--FR-12 dưới góc nhìn tác nhân. Người dùng khởi tạo mọi luồng nghiệp vụ; Dịch vụ AI ngoài chỉ hỗ trợ trích xuất và diễn giải ở các ca nhập liệu, phân tích và insight; Worker nền phụ trách các ca phân tích định kỳ, khoản định kỳ và dọn export; Quản trị viên phát triển chỉ cấu hình hệ thống. Các ca sử dụng chi tiết của từng FR được tách thành sơ đồ riêng ở các mục dưới đây.
...
\textbf{Đầu vào và xử lý.} Văn bản không rỗng; hồ sơ có ví và danh mục. Trường đích gồm mô tả, amount dương, loại \texttt{income/expense}, ngày không ở tương lai, danh mục và ví. Orchestrator chọn LLM structured output hoặc parser cục bộ, chuẩn hóa tiền/ngày theo mục~\ref{subsec:amount-date-norm} và ánh xạ danh mục/ví. Backend áp dụng quy tắc sổ cái mục~\ref{sec:fin-data}; LLM chỉ đề xuất đối số. Batch chỉ được ghi nguyên tử sau bản xem trước FR-03.
...
\widereportfigure{15-usecase-fr01.pdf}{Ca sử dụng chi tiết FR-01 --- nhập giao dịch bằng văn bản tự nhiên.}{fig:usecase-fr01}
...
\textbf{Đầu vào và xử lý.} Adapter media phải được cấu hình hoặc có provider cục bộ. Provider trả raw OCR/transcript kèm tên provider; hệ thống hiển thị kết quả, trích xuất trường rồi tạo lựa chọn/bản xem trước. Nền tảng OCR/STT ở mục~\ref{subsec:ocr-stt}; CER/WER và độ đúng trường ở mục~\ref{subsec:ocr-stt-error}. Phiên bản hiện tại có thể cho chọn lưu tổng hóa đơn hoặc từng dòng, nhưng \textbf{chưa đối soát tổng mặt hàng với tổng hóa đơn}.
...
\begin{itemize}
  ...
  \item \texttt{TC-FR02-04}: file sai loại hoособ lớn hơn 10~MB bị chặn trước nghiệp vụ.
\end{itemize}

\widereportfigure{16-usecase-fr02.pdf}{Ca sử dụng chi tiết FR-02 --- nhập giao dịch bằng ảnh và giọng nói.}{fig:usecase-fr02}
...
\widereportfigure{17-usecase-fr03.pdf}{Ca sử dụng chi tiết FR-03 --- clarification và giao dịch chờ xác nhận.}{fig:usecase-fr03}
...
\textbf{Đầu vào và xử lý.} Chuỗi được bỏ dấu, hạ chữ và chuẩn hóa khoảng trắng. Thứ tự là tên exact, alias exact rồi fuzzy theo mục~\ref{subsec:text-similarity}; ngưỡng 0,90 cho input rất ngắn, 0,82 cho input còn lại và margin tối thiểu 0,08. Correction gần input có thể làm few-shot context; kết quả vẫn qua validation/xác nhận. Sửa category của giao dịch không-manual ghi cặp cũ--đúng best-effort sau commit.
...
\widereportfigure{18-usecase-fr04.pdf}{Ca sử dụng chi tiết FR-04 --- phân loại danh mục và học từ phản hồi.}{fig:usecase-fr04}
...
\widereportfigure{19-usecase-fr05.pdf}{Ca sử dụng chi tiết FR-05 --- quản lý giao dịch, ví và danh mục.}{fig:usecase-fr05}
...
\textbf{Đầu vào và xử lý.} Service khóa ví theo thứ tự, trừ $A$ ở nguồn, cộng $A$ ở đích và ghi \texttt{wallet\_transfers} trong cùng \texttt{BEGIN/COMMIT}. Chuyển nội bộ giữ bất biến tổng thay đổi bằng 0 theo mục~\ref{subsec:ledger-model}. Lãi/lỗ đầu tư khác 0 chỉ áp dụng cho ví investment/savings.
...
\widereportfigure{20-usecase-fr06.pdf}{Ca sử dụng chi tiết FR-06 --- chuyển ví và dòng tiền đặc biệt.}{fig:usecase-fr06}
...
\textbf{Đầu vào và xử lý.} Chỉ dùng giao dịch chưa xóa và zero-fill trục thời gian khi cần. Analytics Engine áp dụng mục~\ref{subsec:ols-trend}--\ref{subsec:correlation}: (i) trend chỉ công bố tăng với ít nhất 3 tháng, slope dương, tăng trung bình ít nhất 10\% và $R^2\geq0{,}5$; (ii) anomaly cần ít nhất 4 điểm, cờ phía chi lớn khi $z\geq2{,}5$ hoặc vượt $Q_3+1{,}5IQR$; (iii) runway dùng đúng 14 ngày lịch kể cả ngày chi 0, số dư không dương trả 0 ngày và burn rate 0 không dự báo ngày cạn; (iv) recurring dùng cửa sổ 90 ngày, amount tối đa 500.000~VND, sai số 15\%, cadence 20--40 ngày; (v) correlation zero-fill 12 tuần, cần dữ liệu ở ít nhất 4 tuần và chỉ công bố tương quan \textbf{dương} $r\geq0{,}6$, không dùng $|r|$.
...
\widereportfigure{21-usecase-fr07.pdf}{Ca sử dụng chi tiết FR-07 --- phân tích dữ liệu tài chính.}{fig:usecase-fr07}
...
\textbf{Đầu vào và xử lý.} Backend gửi facts có cấu trúc cho narrator, đồng thời tạo lời khuyên tổng quan bằng quy tắc xác định. Persona chỉ thay cách diễn đạt; response luôn kèm facts, provider và metadata để đối chiếu. Ranh giới ở mục~\ref{subsec:llm-boundary-theory}, phép đo grounding ở mục~\ref{subsec:algo-grounding-perf}.
...
\widereportfigure{22-usecase-fr08.pdf}{Ca sử dụng chi tiết FR-08 --- insight có căn cứ và persona.}{fig:usecase-fr08}
...
\widereportfigure{23-usecase-fr09.pdf}{Ca sử dụng chi tiết FR-09 --- ngân sách, đề xuất và dự báo.}{fig:usecase-fr09}
...
\textbf{Đầu vào và xử lý.} Goal Planner áp dụng mục~\ref{subsec:goal-planning} để tính phần thiếu, số tháng, mức góp đến hạn, tiến độ và cảnh báo. Với nợ, lãi suất tháng bằng lãi suất năm chia 12; công thức niên kim có nhánh lãi suất 0 và mô phỏng tháng để tính lãi/remaining balance; khoản trả không vượt lãi tháng đầu bị cờ negative amortization. What-if chạy lại cùng hàm. Endpoint plan không ghi DB, cấp token ký theo payload trong 15 phút; tạo/cập nhật trường kế hoạch chỉ nhận token còn hạn và fingerprint khớp.
...
\widereportfigure{24-usecase-fr10.pdf}{Ca sử dụng chi tiết FR-10 --- mục tiêu tài chính và mô phỏng what-if.}{fig:usecase-fr10}
...
\widereportfigure{25-usecase-fr11.pdf}{Ca sử dụng chi tiết FR-11 --- khoản định kỳ và tác vụ chủ động.}{fig:usecase-fr11}
...
\widereportfigure{26-usecase-fr12.pdf}{Ca sử dụng chi tiết FR-12 --- xuất dữ liệu, lịch sử tệp và dọn dẹp.}{fig:usecase-fr12}

\subsection{Yêu cầu phi chức năng}

Các ngưỡng dưới đây là tiêu chí nghiệm thu hoặc thiết kế đích, không mặc nhiên là kết quả hiện tại. Kết quả thực đo và phép đo còn thiếu được tách tại mục~\ref{subsec:results}.
...
Hình~\ref{fig:domain-class} tách bốn aggregate: hồ sơ và cá nhân hóa (\code{AIPersonality}, \code{UserTrait}), sổ cái (\code{Wallet}, \code{Transaction}, \code{Category}), kế hoạch và khoản định kỳ (\code{Budget}, \code{FinancialGoal}, \code{RecurringBill}, \code{RecurringPayment}), hội thoại và phản hồi (\code{ChatMessage}, \code{AIFeedback}). Thực thể \code{User} được vẽ một lần duy nhất, đặt riêng phía trên bốn khung và là gốc sở hữu của cả bốn aggregate. Quan hệ ghi đủ ký hiệu UML: hình thoi đặc ở đầu \code{User} cho quan hệ composition tới \code{Wallet}, \code{Budget}, \code{UserTrait} và \code{ChatMessage}; hình thoi rỗng cho quan hệ aggregation tới \code{AIPersonality} vì persona tồn tại độc lập với hồ sơ đang bật nó; mũi tên hở thể hiện chiều đọc; bội số ghi ở cả hai đầu kèm role name. Hai tham chiếu lỏng qua khóa ngoại nullable (\code{AIPersonality} tạo văn phong cho \code{ChatMessage}, \code{AIFeedback} sửa nhãn của \code{Transaction}) vẽ bằng nét đứt để phân biệt với quan hệ sở hữu. Mọi đường đều định tuyến vuông góc, không bo cung. Các service phân tích, ngân sách và mục tiêu không xuất hiện trong hình vì đây là mô hình dữ liệu miền; chúng chỉ đọc aggregate để tính facts hoặc kế hoạch và không sở hữu entity tài chính nào.
...
\widereportfigure{06-llm-boundary.pdf}{Ranh giới trách nhiệm của LLM: dữ liệu và facts được tạo ở lõi xác định; LLM chỉ chuyển đổi ngữ nghĩa và diễn giải.}{fig:llm-boundary}
...
\widereportfigure{07-conversation-state.pdf}{Máy trạng thái hội thoại và giao dịch chờ xác nhận.}{fig:conversation-state}
...
\widereportfigure{09-multimodal-flow.pdf}{Luồng xử lý đầu vào đa phương thức. Voice và ảnh hội tụ vào pipeline text chung sau bước xác nhận.}{fig:multimodal-flow}
...
\widereportfigure{10-feedback-flow.pdf}{Luồng phản hồi và cá nhân hóa phân loại bằng correction retrieval, không phải fine-tuning.}{fig:feedback-flow}

Theo Hình~\ref{fig:feedback-flow}, khi người dùng sửa category, hệ thống lưu cặp kết quả AI--kết quả đúng. Lần sau, correction exact/fuzzy được xét trước, sau đó mới đến matcher/LLM. Giao dịch lặp trong ``Khác'' có thể được gom cụm để đề xuất category; tạo category và re-tag luôn là kế hoạch chờ xác nhận. Lịch sử mâu thuẫn phải hạ confidence thay vì học theo correction gần nhất.
...
\widereportfigure{12-goal-flow.pdf}{Luồng lập kế hoạch mục tiêu và mô phỏng what-if.}{fig:goal-flow}
...
Trong Hình~\ref{fig:worker-sequence}, scheduler tạo job có định danh; worker lấy đúng user scope, gọi handler, tính facts và ghi internal message hoặc export. Retry dùng cùng fingerprint; unique event key và thao tác idempotent loại thông báo trùng. Tính năng hiện tại tạo thông báo trong \code{chat_messages}, không phải push notification. Auto-backup theo lịch chưa có worker tương ứng nên không được mô tả là đã hoạt động.
...
Nguyên mẫu chưa có authentication production; vì vậy không được tuyên bố đã cô lập nhiều người dùng. Các route dùng \code{default_user}. Mọi thao tác theo ID đã được rà soát để luôn kèm điều kiện \code{user_id} và có test truy cập chéo (Bảng~\ref{tab:idor-verification}), nhưng điều kiện dữ liệu không thay cho xác thực: trước triển khai thật vẫn cần bổ sung authentication và authorization thực sự.

\begin{longtable}{@{}L{5.2cm}L{2.2cm}L{5.2cm}@{}}
  \caption{Kiểm thử truy cập chéo theo ID (ngày 25/07/2026)}\label{tab:idor-verification}\\
  \toprule
  \textbf{Endpoint bị thử} & \textbf{Kết quả} & \textbf{Diễn giải} \\
  \midrule
  \endfirsthead
  \toprule
  \textbf{Endpoint bị thử} & \textbf{Kết quả} & \textbf{Diễn giải} \\
  \midrule
  \endhead
  \code{GET/PUT/DELETE /api/budgets/:id} & 404 & Không đọc, sửa hoặc xóa được ngân sách của người dùng khác. \\
  \code{GET/PUT/DELETE /api/recurring/:id} & 404 & Tương tự với khoản định kỳ. \\
  \code{POST} tới \code{:id/pause}, \code{:id/resume}, \code{:id/pay} & 404 & Không đổi trạng thái hoặc ghi thanh toán lên bản ghi người khác. \\
  \code{GET} tới \code{:id/payments} & 404 & Trước khi thêm kiểm tra sở hữu ở route, endpoint trả 200 kèm lịch sử rỗng: không rò dữ liệu nhưng vẫn tiết lộ id có tồn tại. \\
  Nhóm chứng: bản ghi của chính mình & 200 & Quyền truy cập hợp lệ không bị ảnh hưởng. \\
  \bottomrule
\end{longtable}

Phương pháp: tạo hồ sơ thứ hai, chèn bản ghi thuộc hồ sơ đó, rồi gọi API bằng \code{default_user} với đúng ID vừa tạo; 10/10 lượt truy cập chéo bị chặn và 2/2 lượt truy cập hợp lệ vẫn hoạt động. Bốn ca hồi quy trong \code{tests/user-idor-scope.test.js} kiểm tra trên \emph{câu SQL phát ra} chứ không chỉ giá trị trả về, vì khi hệ thống chỉ có một hồ sơ demo thì truy vấn thiếu điều kiện \code{user_id} vẫn trả đúng bản ghi. Artifact: \code{log/idor-verification_2026-07-25.json}.
...
\begin{longtable}{@{}L{3.3cm}L{4.2cm}L{2.4cm}L{3.4cm}@{}}
  ...
  \midrule
  \endhead
  Backend \code{npm test} sau sửa & 182/182 test đạt, 0 thất bại & \statusmeasured & Unit/service trên 38 tệp test; gồm transaction, concurrency, recurring, health, importer, time-series và user scope. Mốc 16/07/2026 là 100/100 trước khi mở rộng bộ test. \\
  ...
  Phân loại danh mục (parser cục bộ) & Acc 29,36\%; macro-F1 0,177 (12 lớp) & \statusmeasured & 5.265 dòng gán nhãn; nhãn lịch sử theo nguồn tiền nên chỉ độc lập một phần về lược đồ. \\
  Ablation parser vs LLM (Gemini) & Parser 22,2\%/0,204; LLM 59,5\%/0,607 & \statusmeasured & Mẫu phân tầng 63 câu; 63/63 gọi Gemini thành công; p50 964 ms. \\
  Feedback before/after & Acc 0\% $\to$ 68,19\%; macro-F1 0 $\to$ 0,544 & \statusmeasured & 3.502 câu parser-sai chia seed/holdout; kiểm tra không suy giảm nhóm chứng. \\
...
\begin{longtable}{@{}L{3.2cm}L{2.5cm}L{4.3cm}L{3.5cm}@{}}
  ...
  Local parser & \statusmeasured & 31/31 strict trên gate cố định; trên 5.265 dòng độc lập chỉ đạt 29,36\% acc / macro-F1 0,177. & Cải thiện danh mục ít mẫu và câu nguồn-tiền mơ hồ. \\
  LLM tool routing & \statusmeasured & Ablation: Gemini 59,5\%/0,607 so parser 22,2\%/0,204 trên cùng mẫu 63 câu. & Mở rộng mẫu và khóa chi phí/độ trễ theo provider. \\
  ...
  Auth/multi-user & Ngoài phạm vi hiện tại & \code{default_user}; mọi thao tác theo ID đã scoped và có test truy cập chéo, nhưng chưa có xác thực. & Không demo như đã có; thêm xác thực và phân quyền trước production. \\
  \bottomrule
\end{longtable}

\subsubsection{Thí nghiệm định lượng trên tập gán nhãn}

Đợt nghiệm thu ngày 16/07/2026 (mục trên) xác nhận hệ thống chạy đúng ở mức runtime nhưng chưa lượng hóa chất lượng giải thuật trên tập độc lập; do đó một đợt đo thứ hai ngày 24/07/2026 bổ sung ba thí nghiệm định lượng trên toàn bộ tập gán nhãn để trả lời riêng câu hỏi ``giải thuật chính xác đến đâu''. Ba thí nghiệm được thực hiện ngày 24/07/2026 trên chính \code{dataFinance.csv} (5.265 dòng, SHA-256 \code{a9b7cf1b\ldots027ca94f}), commit \code{5f03476}, Node v24.16.0, provider \code{gemini}, model \code{gemini-3.1-flash-lite}. Mã và artifact tái lập (JSON + Markdown) nằm ở \code{demo/backend/tests/experiments/} và \code{log/}.

\paragraph{Benchmark phân loại danh mục.} Chạy \code{parseLocalTransaction} trên toàn bộ 5.265 dòng, so nhãn dự đoán với nhãn gold ánh xạ từ taxonomy lịch sử.

\begin{longtable}{@{}L{6cm}L{4.5cm}@{}}
  \caption{Kết quả phân loại local parser trên \code{dataFinance.csv}}\label{tab:exp-classification}\\
  \toprule
...
Con số thấp là \textbf{phát hiện có ý nghĩa, không phải thất bại}: nhãn lịch sử phân loại theo \emph{nguồn tiền} (ví dụ ``mẹ cho tiền ăn'' $\to$ nhãn gia đình ``Khác'') trong khi parser phân loại theo \emph{nội dung} ($\to$ ``Ăn uống''). Hai lược đồ khác trục nên 79,5\% (2.957/3.719) ca sai xoay quanh lớp ``Khác''. Đây chính là lý do thiết kế chọn LLM làm lớp trích xuất chính và giữ người dùng trong vòng xác nhận, thay vì tin vào so khớp alias.

\paragraph{Ablation parser cục bộ vs LLM.} Trên mẫu phân tầng 63 câu (5 câu/lớp, seed 42), so hai nhánh trích xuất danh mục; nhánh LLM gọi \code{parseWithGemini} trực tiếp, bỏ qua cache và định tuyến cục bộ.

\begin{longtable}{@{}L{2.6cm}R{1.6cm}R{1.6cm}R{1.7cm}R{1.7cm}R{1.2cm}@{}}
  \caption{Ablation local parser vs LLM (Gemini)}\label{tab:exp-ablation}\\
  \toprule
  \textbf{Nhánh} & \textbf{Acc} & \textbf{Macro-F1} & \textbf{Clarify} & \textbf{p50 (ms)} & \textbf{API} \\
  \midrule
  \endfirsthead
  \toprule
  \textbf{Nhánh} & \textbf{Acc} & \textbf{Macro-F1} & \textbf{Clarify} & \textbf{p50 (ms)} & \textbf{API} \\
...
LLM khái quát tốt hơn khoảng 3,0$\times$ về macro-F1 trên câu tự do, đổi lấy độ trễ p50 964 ms và chi phí gọi API. Cả hai nhánh đều đi qua bước xác nhận trước khi ghi, nên LLM là lớp hỗ trợ trích xuất chứ không phải nguồn chân lý. Cả 63/63 lượt gọi Gemini đều thành công (không lỗi API); trong đó 42 câu nhận được nhãn xác định và 21 câu LLM trả về rỗng, được tính là phân loại sai.

\paragraph{Feedback before/after (correction retrieval).} Loại lớp ``Khác'' còn 4.832 câu nội dung. Nhóm parser-sai (3.502 câu) chia đôi: seed (ghi corrections) và holdout (phát lại). Đo cải thiện trên holdout và kiểm tra không suy giảm ở nhóm chứng (1.330 câu parser vốn đúng).

\begin{longtable}{@{}L{3.5cm}R{2.2cm}R{2.6cm}R{2cm}@{}}
  \caption{Feedback before/after trên tập holdout}\label{tab:exp-feedback}\\
  \toprule
  \textbf{Chỉ số} & \textbf{Trước} & \textbf{Sau} & \textbf{$\Delta$} \\
  \midrule
  \endfirsthead
  \toprule
  \textbf{Chỉ số} & \textbf{Trước} & \textbf{Sau} & \textbf{$\Delta$} \\
...
1.194 câu chuyển sai$\to$đúng qua tra cứu tương đồng văn bản (không phải khớp chính xác, nên đo được khả năng khái quát sang câu gần giống). Nhóm chứng có 466/1.330 câu bị correction làm sai, nhưng 430 trong số đó (92\%) là \textbf{nhiễu nhãn cố hữu} của dữ liệu lịch sử --- cùng một câu chữ xuất hiện với nhiều nhãn gold khác nhau, không hệ thống nào phân biệt được. Suy giảm thực trên câu đơn nghĩa chỉ 36 ca (2,7\%), xác nhận correction không học sai một cách hệ thống.

\paragraph{Grounded narration.} Chưa đo định lượng trong đợt này; giữ ở mức thiết kế (cấp cùng facts cho nhiều persona, kiểm tra số liệu giữ nguyên khi giọng điệu thay đổi) và là hướng mở rộng ưu tiên. Ba kết quả định lượng còn lại có log tái lập nên được phản ánh trong tóm tắt và kết luận.
