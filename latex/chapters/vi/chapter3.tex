...
\begin{longtable}{@{}L{3.3cm}L{4.2cm}L{1.8cm}L{3.4cm}@{}}
  ...
  Ablation parser vs LLM (Gemini) & Parser 21,6\%/0,192; LLM 54,3\%/0,561 & \statusmeasured & Mẫu phân tầng 51 câu; 51/51 gọi Gemini thành công; p50 897 ms. \\
...
\begin{longtable}{@{}L{3.2cm}L{2.5cm}L{3.5cm}L{3.5cm}@{}}
  ...
  LLM tool routing & \statusmeasured & Ablation: Gemini 54,3\%/0,561 so parser 21,6\%/0,192 trên cùng mẫu 51 câu. & Mở rộng mẫu và khóa chi phí/độ trễ theo provider. \\
...
\paragraph{Ablation parser cục bộ vs LLM.} Trên mẫu phân tầng 51 câu (4 câu/lớp, seed 42), so hai nhánh trích xuất danh mục; nhánh LLM gọi \code{parseWithGemini} trực tiếp, bỏ qua cache và định tuyến cục bộ.
...
LLM khái quát tốt hơn khoảng 2,9$\times$ về macro-F1 trên câu tự do, đổi lấy độ trễ p50 897 ms và chi phí gọi API. Cả hai nhánh đều đi qua bước xác nhận trước khi ghi, nên LLM là lớp hỗ trợ trích xuất chứ không phải nguồn chân lý. Đo trên 51/51 câu gọi Gemini thành công.
