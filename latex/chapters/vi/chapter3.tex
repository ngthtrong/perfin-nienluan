...
\begin{longtable}{@{}L{5.2cm}L{2.2cm}L{5.2cm}@{}}
  ...
  \code{POST /api/recurring/:id/pause|resume|pay} & 404 & Không đổi trạng thái hoặc ghi thanh toán lên bản ghi người khác. \\
  \code{GET /api/recurring/:id/payments} & 404 & Trước khi thêm kiểm tra sở hữu ở route, endpoint trả 200 kèm lịch sử rỗng: không rò dữ liệu nhưng vẫn tiết lộ id có tồn tại. \\
...
\begin{longtable}{@{}L{3.2cm}L{2.5cm}L{4.3cm}L{3.5cm}@{}}
  ...
  Auth/multi-user & Ngoài phạm vi hiện tại & \code{default_user}; mọi thao tác theo ID đã scoped và có test truy cập chéo, nhưng chưa có xác thực. & Không demo như đã có; thêm authentication/authorization trước production. \\
  \bottomrule
\end{longtable}
