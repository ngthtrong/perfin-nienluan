...
\widereportfigure[][Hình~\ref{fig:system-context} cho thấy phạm vi niên luận là thu nhận, tổ chức và phân tích dữ liệu tài chính cá nhân. Kết nối ngân hàng, ví dùng chung, xác thực production và hạ tầng sẵn sàng cao không thuộc phạm vi thực hiện.]{figures/drawio/01-system-context.png}{Ngữ cảnh và phạm vi của PERFIN; các dịch vụ LLM, OCR và STT nằm ngoài biên tin cậy dữ liệu.}{fig:system-context}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}L{1.0cm}Y Y L{2.0cm}@{}}
    ...
    O5 & Đánh giá nguyên mẫu bằng chiến lược kiểm thử lai tối giản. & Dùng test tự động cho lõi dữ liệu--giải thuật và ca chức năng thủ công cho luồng chính; nêu rõ ranh giới mock/live. & Tuần 13 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{Kiểm thử:} dùng unit/hồi quy trên fixture có kiểm soát, kiểm tra tích hợp nội bộ bằng mock hoặc bộ nhớ trong, và các kịch bản chức năng thủ công do tác giả thực hiện. Kết quả được giới hạn đúng phạm vi, không suy rộng kiểm thử thủ công thành tính dễ dùng hoặc độ chính xác OCR/STT.
\end{enumerate}
...
\begin{longtable}{@{}C{1.0cm}L{2.7cm}L{6.2cm}L{4.0cm}@{}}
  ...
  11 & 20--26/07 & Chạy kiểm thử lõi tự động, lập ca chức năng thủ công và profile dữ liệu. & Log kiểm thử, danh sách ca thủ công và tóm tắt dữ liệu. \\
...
Các đóng góp chính gồm: mô hình dữ liệu có ràng buộc và cập nhật nguyên tử; pipeline đa phương thức có con người trong vòng lặp; nhóm giải thuật phân tích có thể kiểm tra độc lập; ranh giới trong đó LLM chỉ trích xuất và diễn giải; và chiến lược kiểm thử lai tách tính đúng của lõi khỏi việc kiểm tra các luồng giao diện.
