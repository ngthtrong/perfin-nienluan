...
\widereportfigure{figures/drawio/14-usecase-overview.png}{Overall PERFIN use case diagram. Feature details are specified with flow tables instead of twelve separate diagrams.}{fig:usecase-overview}
...
\widereportfigure{figures/drawio/04-domain-class.png}{PERFIN UML domain class model separating ledger, planning, recurring and conversation/feedback aggregates.}{fig:domain-class}
...
\widereportfigure{figures/drawio/05-physical-erd.png}{Physical ERD reconciled with runtime migrations; principal business foreign keys use separate orthogonal corridors.}{fig:physical-erd}
...
\widereportfigure{figures/drawio/06-llm-boundary.png}{LLM responsibility boundary: typed drafts and narration remain outside validation, calculation and data-write authority.}{fig:llm-boundary}
...
\widereportfigure{figures/drawio/08-text-sequence.png}{Text-entry sequence; the data-write transaction starts only after confirmation.}{fig:text-sequence}
...
\widereportfigure{figures/drawio/09-multimodal-flow.png}{Image and speech processing; both branches converge on the text pipeline after raw-text review.}{fig:multimodal-flow}
...
\widereportfigure{figures/drawio/10-feedback-flow.png}{Classification and correction-retrieval flow; feedback is stored only after confirmation.}{fig:feedback-flow}
...
\widereportfigure{figures/drawio/11-insight-sequence.png}{Grounded-insight sequence; Analytics returns facts before persona/LLM narration.}{fig:insight-sequence}
...
The images in this subsection were regenerated on 13 August 2026 from the current Expo web snapshot at a $390\times844$ CSS-pixel viewport, device scale factor 2 and synthetic demo data. Chat history is isolated so that the preview state remains legible. The images illustrate design decisions; they are not manual-test results, native-device screenshots or UAT evidence.
...
The two commands in Table~\ref{tab:automated-test-results} were rerun on 13 August 2026 against the current source snapshot using Node.js v24.16.0, with Redis and jobs disabled and the local parser selected. Results are reported in the unit produced by each command.
...
\textbf{Status note:} The ``Pass'' values marked with an asterisk in Table~\ref{tab:manual-test-cases} are draft assumptions, not verified results. Before submission, the author must execute every case, update its status from the observed result, and only then remove this note. Manual testing by the author is not UAT and does not establish OCR/STT accuracy, usability or performance.
...
\begin{longtable}{@{}L{1.5cm}L{4.6cm}L{6.5cm}C{1.5cm}@{}}
  ...
  \midrule
  \endhead
  % TODO(TC-M01): execute, save evidence and update status before submission.
  \code{TC-M01} & Create, edit, delete and restore a transaction through the form. & The transaction list and wallet balance reflect each operation correctly. & \textbf{Pass}\textsuperscript{*} \\
  % TODO(TC-M02): execute, save evidence and update status before submission.
  \code{TC-M02} & Enter a complete transaction through chat, review its preview and confirm it. & Exactly one record is created, and only after confirmation. & \textbf{Pass}\textsuperscript{*} \\
  % TODO(TC-M03): execute, save evidence and update status before submission.
  \code{TC-M03} & Enter incomplete or ambiguous content, then cancel. & The system requests missing information and writes no transaction. & \textbf{Pass}\textsuperscript{*} \\
  % TODO(TC-M04): execute, save evidence and update status before submission.
  \code{TC-M04} & Submit a receipt image and a voice recording in turn; review the raw text. & Raw text can be edited or confirmed, and no data is saved automatically before confirmation. & \textbf{Pass}\textsuperscript{*} \\
  % TODO(TC-M05): execute, save evidence and update status before submission.
  \code{TC-M05} & Open Dashboard/Report, reconcile income and expense totals, and export CSV. & Totals match the demo data; the CSV downloads and opens successfully. & \textbf{Pass}\textsuperscript{*} \\
  % TODO(TC-M06): execute, save evidence and update status before submission.
  \code{TC-M06} & Create a budget for a period containing transactions. & Spent and remaining amounts match transactions in the period. & \textbf{Pass}\textsuperscript{*} \\
  % TODO(TC-M07): execute, save evidence and update status before submission.
  \code{TC-M07} & Create a goal and run a what-if scenario. & The simulation is displayed without automatically changing the original plan. & \textbf{Pass}\textsuperscript{*} \\
  % TODO(TC-M08): execute, save evidence and update status before submission.
  \code{TC-M08} & Create a recurring bill and record one payment. & Status is updated and no duplicate transaction is created. & \textbf{Pass}\textsuperscript{*} \\
  \bottomrule
\end{longtable}
